          ...
          return (
            <div className="flex flex-col gap-6">
              {Object.entries(byNs).sort(([a], [b]) => a.localeCompare(b)).map(([ns, items]) => (
                <div key={ns} className="rounded-lg border border-zinc-300 bg-zinc-50 shadow-sm dark:border-zinc-700 dark:bg-zinc-900/40">
                  <div className="flex items-center justify-between rounded-t-lg border-b border-zinc-300 bg-zinc-200/70 px-4 py-2.5 dark:border-zinc-700 dark:bg-zinc-800/60">
                    <span className="text-sm font-semibold text-zinc-700 dark:text-zinc-200">
                      Namespace: <span className="font-bold text-zinc-900 dark:text-white">{ns}</span>
                    </span>
